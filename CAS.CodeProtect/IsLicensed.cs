//<summary>
//  Title   : Helper class geting access to a license for the particular type.
...
{
  /// <summary>
  /// Helper interface geting access to the basic information about the license.
  /// This interface might be usefull because class IsLicensed&lt;type&gt; is generic
  /// </summary>
  public interface IIsLicensed
  {
    /// <summary>
    /// Gets or sets the volumen constrain.
    /// </summary>
    /// <value>The volumen. Null if not valid license found.</value>
    int? Volumen { get;  }
    /// <summary>
  ...
  /// <summary>
  /// Helper class geting access to a license for the particular type.
  /// </summary>
  ...
  public abstract class IsLicensed<type>: IIsLicensed where type: class
  {
    ...
    /// <see cref="TraceNoLicenseFile"/>, <see cref="TraceCurrentLicence"/> to provide more information about the validation process.
    /// </summary>
    /// <param name="defalutVolumen">The default volume assigned if license does not have valid constrain for the type.</param>
    ...
    /// <item><see cref="TraceCurrentLicence"/> - is called if the license is valid.</item>
    /// <item><see cref="TraceWarning"/> - is called only if license is valid and warning is provided.</item>
    /// </list>
    /// </remarks>
    public IsLicensed( int? defalutVolumen, TimeSpan? defaultRunTime )
    {
      Debug.Assert
        ( typeof( type ).IsSealed, "Implementation Error, The IsLicensed<type> generic class is valid only for types (type) that are sealed." );
      Volumen = defalutVolumen;
      ...
      using ( m_license )
      {
        ...
        Licensed = true;
        Volumen = m_license.VolumeConstrain >= 0 ? m_license.VolumeConstrain : int.MaxValue;
        RunTime = m_license.RunTimeConstrain >= 0 ? TimeSpan.FromHours( m_license.RunTimeConstrain ) : TimeSpan.MaxValue;
        TraceCurrentLicence( m_license );
    ...
    /// </remarks>
    protected virtual void TraceCurrentLicence( LicenseFile license )
    {
